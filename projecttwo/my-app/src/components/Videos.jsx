...
const Videos = () => {
    ...
    const [videoSrc, setVideSrc] = useState(videosArr[0]);
    return <Stack direction={['column', 'row']} h={'100vh'}>
        ...
        <VStack w={'full'}>
            ...
            <VStack alignItems={'flex-start'} p={'8'}  w={'full'} overflowY={'auto'}>
                <Heading>Sample video a</Heading>
                <Text>This is a sample video for testing and this is description...</Text>
            </VStack>
        </VStack>
        ...
        <VStack w={["full", 'xl']} alignItems={'stretch'} p={'8'} spacing={'8'} overflow={'auto'}>

            {
                videosArr.map((item, index) => (
                    <Button variant={'ghost'} colorScheme={'purple'} onClick={() => setVideSrc(item)}>Lecture {index + 1}</Button>
                ))
            }
